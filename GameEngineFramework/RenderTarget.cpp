...
void RenderTarget::Create(unsigned int _width, unsigned int _height) {
	...
	m_texture = SDL_CreateTexture(Renderer::Instance().GetRenderer(), SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, m_width, m_height);
...
void RenderTarget::Render(float _deltaTime) {
	...
	int xRes = ws.X + (NATIVE_XRES * perc);
